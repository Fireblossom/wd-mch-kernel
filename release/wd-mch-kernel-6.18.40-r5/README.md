# WD My Cloud Home（RTD1295）Linux 6.18.40 刷机包 r5
...
r5 已在一台单盘版设备上刷入并验证：四核 SMP、串口真中断、千兆网、Docker、
OpenMediaVault 8、USB 3.0 5 Gbit/s、md 阵列组装与 `/data` 挂载、硬件看门狗
（systemd 接管，15 秒超时）、pstore 崩溃日志（ramoops）、AES 硬件加速
（dm-crypt aes-xts 实测写 192 / 读 276 MB/s，同设备明文基线 276 / 400 MB/s）、
温度经 thermal 与 hwmon（`sensors`）双路径可读、软重启，以及连续三次冷断电
（36–39 秒回网）全部自愈。

但请注意这句话的边界：

- **验证时长以小时计，不是长期运行验证。** 本项目的任何主线内核版本（含更早的
  `r1`/`r2`）都没有经历过长期真机使用；唯一经过多年实际使用检验的，是厂商的 4.9 内核。
...
| 版本 | 内容 |
|---|---|
| r3 | pstore/ramoops（崩溃日志跨重启保存）+ kexec |
| r4 | 硬件看门狗修复（此前驱动 options 缺标志导致 systemd 无法喂狗）+ DTS bootargs 清理 |
| r5 | AES/GHASH ARMv8 CE 硬件加速（加密卷提速约 5–7 倍）+ 温度暴露到 hwmon/`sensors` |
...
| SHA-256 和归档内容检查 | 已通过 |
| 真机冷断电×3、SMP、网络、USB、存储、加密、看门狗和软重启 | 已通过（范围见上） |

## 包内文件

`flash/` 下的固件文件是一套，不能和其他版本（`r1`/`r2` 或旧 `vNN`）混用：
...
- （可选）`cryptsetup benchmark` 的 aes-xts 行应在 180 MiB/s 以上（单线程）；
- 软重启能够再次进入同一系统。
...
- `poweroff` 没有断电实现（无 `pm_power_off`）：关机会停在 halt，不会切断电源，
  需要外部断电；`reboot` 正常工作；
- 前面板 LED 无驱动，不可控；
- 无 cpufreq/DVFS，CPU 恒定全速运行（功耗与发热恒定，thermal 有监控）；
